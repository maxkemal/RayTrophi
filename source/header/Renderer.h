...
private:
    ...
    std::mutex cache_mutex;  // Header'da tanýmlama
    ...
    static const int MAX_DIMENSIONS = 2; // Halton dizisi için maksimum boyut
     size_t MAX_SAMPLES_HALTON = 1024;  // 8K için yeterli olacak þekilde
    int max_halton_index ; // Örnek bir deðer, ihtiyaca göre ayarlayýn
     const int MAX_SAMPLES_SOBOL = 4;
   
    std::vector<std::vector<float>> sobol_cache;
    std::unique_ptr<float[]> halton_cache;  // Tek boyutlu array olarak tutacaðýz
    ...
    std::vector<int> sample_budget_map; // Þimdilik opsiyonel
