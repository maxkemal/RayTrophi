...
struct HitRecord {
    ...
    float v;
    bool front_face;
    // Ambient Occlusion için gerekli alanlar
    float ao;           // Ambient Occlusion deðeri
    bool ao_computed;   // AO hesaplandý mý?
    ...
    // AO deðerini sýfýrlama metodu (yeni)
    inline void reset_ao() {
        ao_computed = false;
        ao = 1.0f;
    }
    // Konstruktör
    HitRecord() : t(0), front_face(false), ao(1.0f), ao_computed(false), u(0), v(0) {}
};
