...
public:
    ...
    Dielectric(double index_of_refraction,
        const Vec3& color,
        double caustic_intensity=1.0,          
        double tint_factor=0.1,
        double roughness = 0.0,
        double scratch_density = 0.0);
   ...
   // Vec3 calculate_reflected_attenuation(const Vec3& base_color,const Vec3& fresnel_factor) const;
    ...
    Vec3 calculate_refracted_attenuation(const Vec3& base_color, double thickness, const Vec3& fresnel_factor, const std::array<double, 3>& ior) const;

    virtual bool scatter(const Ray& r_in, const HitRecord& rec, Vec3& attenuation, Ray& scattered) const override;
    virtual double getIndexOfRefraction() const override;

    Vec3 fresnel(const Vec3& incident, const Vec3& normal, const std::array<double, 3>& ior) const;
