...
> **Durum:** AKTIF — Template/acilis deneyimi icin kanonik urun yonu.
...
**Temel ilke:** RayTrophi viewport ağırlıklı, sade ve kendine özgü kalır. Template Hub bu yapının önüne eklenen ağır bir proje yöneticisi değil, kullanıcıyı doğru üretim bağlamına hızla yerle�Son güncelleme: **2026-08-17**
...
| Faz 3 — Sade Template Hub | **CONFIRMED** (2026-08-15 Kullanıcı Doğrulaması) | `TemplateHubUI` ImGui overlay, 3-sütun boyutlandırılabilir düzen, kategori sekmeleri, arama filtresi, görünmez boyutlandırma ayraçları (`Invisible Splitters`), her şablona özel simge/gradyan kimliği, `recent_projects.json` kaydı ve dosya diyaloğu açmadan doğrudan recent proje yükleme teslim edildi ve başarıyla derlenip çalıştırıldı. |
| Faz 4 — Üretim ve VFX template'leri | **CONFIRMED** (2026-08-15 Kullanıcı Doğrulaması) | Altı kanonik üretim template preset builder'ına ek olarak `Gas & Smoke Simulation` (`raytrophi.vfx.gas_smoke`) ve `Fluid Studio` (`raytrophi.vfx.fluid_studio`) VFX recipe paketleri ve prosedürel staged builder'ları doğrulandı ve çalıştırıldı. |
| Faz 5 — Recent, Recovery ve Başlangıç Tercihleri | **CONFIRMED** (2026-08-17) | `StartupPreferencesManager` modülü (`StartupPreferences.h/.cpp`), `assets/config/startup_preferences.json` konfigürasyonu, 4 farklı açılış modu (`ShowHub`, `OpenLastProject`, `StartEmpty`, `RestoreAutosave`), `Recover Autosave` dosya varlığı ve doğrudan oturum kurtarma yüklemesi, Template Hub sidebar `STARTUP ACTION` açılır seçeneği, ve Recent Projects ögeleri için sağ tık bağlam menüsü (`Show in Explorer`, `Remove from Recent`) teslim edildi. |
| Faz 6 — Kullanıcı template'leri ve UX Refaktörü | **CONFIRMED** (2026-08-17 Kullanıcı Doğrulaması) | `UserTemplateManager` modülü (`UserTemplateManager.h/.cpp`), `assets/templates/user_templates/` dizini, `File > Save Scene as Template...` menüsü ve diyaloğu, Template Hub `User Templates` kategorisi, özel altın-amber kart görünümü ve sağ tık ile `Delete User Template` opsiyonu teslim edildi. |
| Faz 6B — Template Hub Görsel Cilalama, Tema Entegrasyonu ve Izgara Zoom | **UYGULANDI — KULLANICI DOĞRULAMASI BEKLİYOR** | Quick Start buton boyutları tek standart boyuta (`34px`) getirildi; `BeginSection` başlık çubukları çerçeveye 1:1 hizalandı; tüm şablon kartları ve sekmeler genel tema sistemine (`ThemeManager`) bağlandı; arama çubuğunun yanına dinamik Grid Scale `[ S | M | L ]` (Compact 165px, Standard 215px, Large 275px) ölçek kontrolü eklendi; kullanıcı şablonları (`User Templates`) için özel Amber-Gold başlık şeridi ve `[USER PRESET]` rozeti entegre edildi. |eslim edildi. |
| Faz 6 — Kullanıcı template'leri ve UX Refaktörü | **UYGULANDI — KULLANICI DOĞRULAMASI BEKLİYOR** | `UserTemplateManager` modülü (`UserTemplateManager.h/.cpp`), `assets/templates/user_templates/` dizini, `File > Save Scene as Template...` menüsü ve diyaloğu, Template Hub `User Templates` kategorisi, özel altın-amber kart görünümü ve sağ tık ile `Delete User Template` opsiyonu teslim edildi; kaba 42px Launch butonu kaldırılarak yerine zarif ipucu metni eklendi. |
