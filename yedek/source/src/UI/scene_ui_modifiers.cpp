    ...
    if (terrain) {
        ...
        if (!rightDockOnly) {
            ...
            if (terrain_brush.mode == 4 && beginBrushDockSection("Stamp##terrain_sculpt_section")) {
                ImGui::Checkbox("Anchored Stroke##terrain", &terrain_brush.stroke_anchored);
                if (ImGui::IsItemHovered()) {
                    ImGui::SetTooltip("Blender-style anchored stamp: press to fix the imprint centre, then\n"
                                      "DRAG to set its depth/radius and rotation live.\n"
                                      "Non-accumulating single imprint.");
                }
    ...
    if (!rightDockOnly) {
        ...
        if (beginBrushDockSection("Alpha Mask")) {
            ...
            if (sculpt_mode_state.tool == SceneUI::SculptBrushTool::Stamp) {
                ImGui::Separator();
                ImGui::Checkbox("Anchored Stroke##sculpt", &sculpt_mode_state.stroke_anchored);
                if (ImGui::IsItemHovered()) {
                    ImGui::SetTooltip(
                        ...
                        "Off = the stamp paints along the stroke path instead.");
                }
            }
