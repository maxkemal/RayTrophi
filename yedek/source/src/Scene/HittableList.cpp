...
bool HittableList::hit(const Ray& r, float t_min, float t_max, HitRecord& rec, bool ignore_volumes) const {
    if (!visible) return false;
    if (bvh_root) {
        return bvh_root->hit(r, t_min, t_max, rec, ignore_volumes);
    }
    else {
        HitRecord temp_rec;
        ...
        for (const auto& object : objects) {
            if (object && object->hit(r, t_min, closest_so_far, temp_rec, ignore_volumes)) {
                hit_anything = true;
