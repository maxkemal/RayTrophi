...
std::vector<uint32_t> loadVolumeSpv(const std::string& path) {
...
bool VulkanBackendAdapter::ensureMaterialPreviewVolumeResources(
    ...
    auto& state = *m_materialPreviewVolume;
    const std::vector<uint32_t> vert =
        loadVolumeSpv(shaderDir + "/material_preview_sky_vert.spv");
    const std::vector<uint32_t> frag =
        loadVolumeSpv(shaderDir + "/material_preview_volume.spv");
    if (vert.empty() || frag.empty()) {
        if (!state.missingShaderReported) {
            state.missingShaderReported = true;
            SCENE_LOG_WARN(
                "[MaterialPreview] Realtime volume shader is missing; "
                "VDB/gas remains available in Vulkan RT until shaders are compiled.");
        }
        return false;
    }
...
void VulkanBackendAdapter::updateMaterialPreviewVolumeBinding() {
    if (!m_materialPreviewVolume || !m_device ||
    ...
    if (volume == VK_NULL_HANDLE ||
        volume == m_materialPreviewVolume->boundVolumeBuffer) return;
    ...
    m_materialPreviewVolume->boundVolumeBuffer = volume;
}

void VulkanBackendAdapter::recordMaterialPreviewVolumePass(
    VkCommandBuffer cmd, const Matrix4x4& viewProj, const Matrix4x4& view,
    uint32_t width, uint32_t height, bool opaqueSnapshotReady) {
    // ★★★ MEASURED 2026-09-04: a live gas domain renders correctly in Vulkan RT
    // and draws NOTHING here, while the volume table published to this backend
    // looks healthy (source_type=5 volume_type=4 grid=SET liveDenseGas=1). The
    // five gates below and the shader are two different failure sites and the
    ...
    const bool gateBound  = m_materialPreviewVolume &&
                            m_materialPreviewVolume->boundVolumeBuffer != VK_NULL_HANDLE;
    SCENE_LOG_ON_CHANGE("mpvolume.gate",
        ...
        std::string("[MPVolume] pass gates: mode=") + (gateMode ? "1" : "0") +
        ...
        (gateMode && gatePipe && gateSet && gateCount && gateBound
            ? "  -> RECORDED (empty frame from here on is the SHADER, not the gates)"
            : "  -> SKIPPED"));
