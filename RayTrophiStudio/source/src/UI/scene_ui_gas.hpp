...
/**
 * @file scene_ui_gas.hpp
 * @brief [DEPRECATED] Legacy UI Panel for Gas/Smoke Simulation
 * @note This legacy UI panel and its associated GasVolume/GasSimulator structures are DEPRECATED.
 * All new simulation features are integrated into the modern "Simulation Panel" (scene_ui_forcefield.hpp -> Domains tab).
 * This file and its references will be completely removed in a future release.
 * 
...
inline void drawGasSimulationProperties(UIContext& ui_ctx, std::shared_ptr<GasVolume> gas) {
    ...
    ImGui::TextWrapped("[DEPRECATED] Bu panel ve bagli oldugu eski GasVolume yapisi kullanimdan kaldirilmistir. Tum yeni gas/smoke simulasyonlari modern 'Simulation Panel' altindaki Domains tabindan yonetilmektedir. Bu legacy yapi ilerde UI'dan tamamen silinecektir.");
    ImGui::PopStyleColor();
    ImGui::Separator();
    ImGui::Spacing();
    ...
    if (UIWidgets::BeginSection("Simulation", ImVec4(0.4f, 0.7f, 1.0f, 1.0f))) {
        ...
        // Backend selection
        const char* backends[] = { "CPU (Dense)", "GPU (CUDA)", "CPU (Sparse OpenVDB)" };
        int current_backend = static_cast<int>(settings.backend);
        
        bool cuda_detected = ::g_hasCUDA;

        if (ImGui::Combo("Backend", &current_backend, backends, 3)) {
