...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Python()
{
	...
	if (!inited)
	{
		...
		static const char* const rtKeywords[] = {
			"rt", "scene", "material", "lights", "render", "project", "timeline", "anim", "nodes", "ui", "addons"
		};
		for (auto& k : rtKeywords)
		{
			Identifier id;
			id.mDeclaration = "RayTrophi API Module";
			langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
		}
