...
struct AnimGraphUIState {
    ...
    uint32_t pendingAutoConnectPinId = 0;
    bool pendingAutoConnectIsOutput = true;
    ...
    int leftPanelTab = 0;  // 0=Playback, 1=Clips, 2=Params
    float leftPlaybackSplitY = 180.0f;
    int rightPanelTab = 0; // 0=Properties, 1=State, 2=Runtime
};
...
inline void requestAnimGraphViewportWake(UIContext& ctx) {
    g_camera_dirty = true;
    g_lights_dirty = true;
    markWorldDirty();
    if (ctx.backend_ptr) {
...
inline void drawAnimGraphSettingsAndStatus(UIContext& ctx, const std::string& characterName, AnimationGraph::AnimationNodeGraph* assetGraph) {
    if (ImGui::CollapsingHeader("Animator Settings", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        ImGui::Spacing();
        RigUI::drawRigPoseViewControls(ctx, characterName, true);
        ImGui::Spacing();
        if (activeModelCtx) {
...
inline void drawAnimationParametersPanel(UIContext& ctx,
    ...
    if (ImGui::Button("Add Parameter") && strlen(newParamName) > 0) {
        switch (paramTypeIdx) {
            ...
            case 3: evalCtx.triggerParams[newParamName] = newParamName; break;
        }
        newParamName[0] = '\0';
    }
...
inline void syncRuntimeNodeFromAsset(AnimationGraph::AnimationNodeGraph* assetGraph,
    ...
    // onLoad may REBUILD dynamic pins: StateMachineNode::onLoad clears inputs and adds one
    // input per state, and those fresh pins come back with id 0. The runtime graph's links
    // address pins by id, so a wiped id disconnects every state input at once - the machine
    // then evaluates to an empty pose and the character freezes the moment its node is
    // selected in the inspector. AnimationNodeGraph::loadFromJson already re-applies pin ids
    // after onLoad for exactly this reason; this is the second call site and needs the same
    // treatment. Asset and runtime are clones of each other, so the asset's pin ids ARE the
    // runtime's ids.
    auto mirrorPinIds = [&](const std::vector<NodeSystem::Pin>& src, std::vector<NodeSystem::Pin>& dst) {
        for (size_t i = 0; i < dst.size(); ++i) {
            if (i < src.size() && src[i].id != 0) {
                dst[i].id = src[i].id;
            } else if (dst[i].id == 0) {
                dst[i].id = runtimeGraph->nextPinId++;
            }
            dst[i].nodeId = runtimeNode->id;
            runtimeGraph->nextPinId = std::max(runtimeGraph->nextPinId, dst[i].id + 1);
        }
    };
    mirrorPinIds(assetNode->inputs, runtimeNode->inputs);
    mirrorPinIds(assetNode->outputs, runtimeNode->outputs);
}
...
inline void drawNodePropertiesPanel(UIContext& ctx,
    ...
    if (g_animGraphUI.selectedNodeIds.empty()) {
        ...
        if (node) {
            ...
            // Same idea for the state machine: which state is live belongs to the runtime
            // instance and is NOT serialized (it used to be, and the asset's stale copy was
            // written back over the runtime's every frame). Read it for display only.
            if (auto* assetSm = dynamic_cast<AnimationGraph::StateMachineNode*>(node)) {
                if (runtimeGraph && runtimeGraph != graph) {
                    if (auto* runtimeSm = dynamic_cast<AnimationGraph::StateMachineNode*>(
                            runtimeGraph->findNodeById(nodeId))) {
                        assetSm->currentStateName = runtimeSm->currentStateName;
                        assetSm->targetStateName = runtimeSm->targetStateName;
                        assetSm->isTransitioning = runtimeSm->isTransitioning;
                        assetSm->transitionProgress = runtimeSm->transitionProgress;
                    }
                }
            }
            ...
            if (node->getTypeId() == "StateMachine") {
                ...
                for (size_t t = 0; t < smNode->transitions.size(); ++t) {
                    ...
                    if (ImGui::TreeNodeEx((trans.fromState + " -> " + trans.toState).c_str(), ImGuiTreeNodeFlags_DefaultOpen)) {
                        ...
                        const char* conds[] = {"None (Exit Time)", "Bool", "Float >", "Float <", "Trigger"};
                        int cType = (int)trans.conditionType;
                        if (ImGui::Combo("Condition Type", &cType, conds, 5)) trans.conditionType = (AnimationGraph::StateMachineNode::Transition::ConditionType)cType;
                        ...
                        if (trans.conditionType == AnimationGraph::StateMachineNode::Transition::ConditionType::Trigger && !trans.parameterName.empty()) {
                            ImGui::SameLine();
                            if (ImGui::Button("Fire Trigger")) {
                                graph->evalContext.triggerParams[trans.parameterName] = trans.parameterName;
                                if (auto* runtimeGraph = getRuntimeGraphForCharacter(ctx.scene, g_animGraphUI.activeCharacter)) {
                                    runtimeGraph->evalContext.triggerParams[trans.parameterName] = trans.parameterName;
                                }
                            }
                        }
                        ...
                        ImGui::SameLine();
                        if (ImGui::Button("Test Transition")) {
                            smNode->currentStateName = trans.fromState;
                            smNode->targetStateName = trans.toState;
                            smNode->transitionProgress = 0.0f;
                            smNode->isTransitioning = true;
                            if (auto* runtimeGraph = getRuntimeGraphForCharacter(ctx.scene, g_animGraphUI.activeCharacter)) {
                                for (auto& node : runtimeGraph->nodes) {
                                    if (auto* rSmNode = dynamic_cast<AnimationGraph::StateMachineNode*>(node.get())) {
                                        rSmNode->currentStateName = trans.fromState;
                                        rSmNode->targetStateName = trans.toState;
                                        rSmNode->transitionProgress = 0.0f;
                                        rSmNode->isTransitioning = true;
                                        break;
                                    }
                                }
                            }
...
inline void drawAnimationPlaybackControls(UIContext& ctx, float panelHeight = 88.0f, bool wrapInChild = true) {
    AnimationController* animCtrl = nullptr;
    AnimationGraph::AnimationNodeGraph* runtimeGraph = nullptr;
    SceneData::ImportedModelContext* activeModelCtx = nullptr;
    ...
    std::snprintf(progressLabel, sizeof(progressLabel), "%d%%", (int)(normalizedTime * 100.0f));
    ImGui::ProgressBar(normalizedTime, ImVec2(-1, 0), progressLabel);
...
inline void drawNodeCanvas(UIContext& ctx, AnimationGraph::AnimationNodeGraph* graph) {
    ...
    // Grid (Noktalı/Çizgili Hibrit Modern Izgara)
    float minorStep = 32.0f * zoom;
    float majorStep = 128.0f * zoom;
    
    // Minor dot grid
    if (minorStep > 12.0f) {
        ImU32 dotColor = IM_COL32(43, 47, 58, 120);
        float startX = fmodf(g_animGraphUI.canvasOffset.x, minorStep);
        float startY = fmodf(g_animGraphUI.canvasOffset.y, minorStep);
        for (float x = startX; x < canvasSize.x; x += minorStep) {
            for (float y = startY; y < canvasSize.y; y += minorStep) {
                if (fmodf(x - g_animGraphUI.canvasOffset.x + 1.0f, majorStep) < minorStep * 0.5f ||
                    fmodf(y - g_animGraphUI.canvasOffset.y + 1.0f, majorStep) < minorStep * 0.5f) {
                    continue;
                }
                drawList->AddCircleFilled(ImVec2(canvasPos.x + x, canvasPos.y + y), 1.0f * std::clamp(zoom, 0.5f, 1.2f), dotColor);
            }
        }
    }
    
    // Major grid lines
    for (float x = fmodf(g_animGraphUI.canvasOffset.x, majorStep); x < canvasSize.x; x += majorStep) {
        drawList->AddLine(
            ImVec2(canvasPos.x + x, canvasPos.y),
            ImVec2(canvasPos.x + x, canvasPos.y + canvasSize.y),
            IM_COL32(58, 64, 78, 140), 1.0f);
    }
    for (float y = fmodf(g_animGraphUI.canvasOffset.y, majorStep); y < canvasSize.y; y += majorStep) {
        drawList->AddLine(
    ...
    for (auto& group : graph->groups) {
        ImVec2 pos = ImVec2(
            canvasPos.x + g_animGraphUI.canvasOffset.x + group.position.x * zoom,
            canvasPos.y + g_animGraphUI.canvasOffset.y + group.position.y * zoom);
        ...
        // Background
        drawList->AddRectFilled(pos, ImVec2(pos.x + size.x, pos.y + size.y), bgCol, radius);
        
        // Border
        bool isSelected = (g_animGraphUI.selectedGroupId == group.id);
        ImU32 borderCol = isSelected 
            ? IM_COL32(255, 180, 50, 255) 
            : ImGui::ColorConvertFloat4ToU32(ImVec4(colF.x, colF.y, colF.z, 0.30f));
            
        drawList->AddRect(pos, ImVec2(pos.x + size.x, pos.y + size.y), borderCol, radius, 0, 
        ...
        ImU32 handleCol = (g_animGraphUI.resizingGroupId == group.id) ? IM_COL32(255, 180, 50, 255) : IM_COL32(160, 160, 170, 120);
        ...
        ImGui::InvisibleButton("AnimGroupResize", ImVec2(resizeMax.x - resizeMin.x, resizeMax.y - resizeMin.y));
        if (ImGui::IsItemActive() && ImGui::IsMouseDragging(ImGuiMouseButton_Left)) {
            g_animGraphUI.resizingGroupId = group.id;
        }
        ImGui::PopID();
        ...
        if (zoom > 0.3f && !group.name.empty()) {
            ...
            drawList->AddRectFilled(badgeMin, badgeMax, badgeCol, radius, ImDrawFlags_RoundCornersTopLeft | ImDrawFlags_RoundCornersBottomRight);
            drawList->AddRect(badgeMin, badgeMax, borderCol, radius, ImDrawFlags_RoundCornersTopLeft | ImDrawFlags_RoundCornersBottomRight, 1.0f);
            
            drawList->AddText(ImVec2(pos.x + badgePaddingX, pos.y + badgePaddingY), 
                IM_COL32(230, 235, 245, 230), group.name.c_str());

            // Group Drag/Click
            ImGui::SetCursorScreenPos(badgeMin);
            ImGui::PushID((int)group.id + 8000000);
            ImGui::InvisibleButton("AnimGroupDrag", ImVec2(badgeMax.x - badgeMin.x, badgeMax.y - badgeMin.y));
            if (ImGui::IsItemActive() && ImGui::IsMouseDragging(ImGuiMouseButton_Left)) {
                g_animGraphUI.selectedGroupId = group.id;
                g_animGraphUI.draggingGroupId = group.id;
            }
    ...
    for (auto& node : graph->nodes) {
        ...
        drawList->AddRectFilled(
            ImVec2(nodePos.x + shadowOffset * 1.5f, nodePos.y + shadowOffset * 1.5f),
            ImVec2(nodeMax.x + shadowOffset * 1.5f, nodeMax.y + shadowOffset * 1.5f),
        ...
        drawList->AddLine(
            ImVec2(nodePos.x + 1.0f, nodePos.y + stripeHeight),
            ImVec2(nodeMax.x - 1.0f, nodePos.y + stripeHeight),
        ...
        drawList->AddRect(nodePos, nodeMax,
            ...
            cornerRadius, 0, isSelected ? 1.25f : 1.0f);
        if (isRuntimeActive) {
        ...
        if (!g_animGraphUI.isCreatingLink && (ImGui::IsItemActivated() || (ImGui::IsItemActive() && ImGui::IsMouseDown(ImGuiMouseButton_Left)))) {
            g_animGraphUI.resizingNodeId = node->id;
        }
        if (g_animGraphUI.isCreatingLink) {
            g_animGraphUI.resizingNodeId = 0;
        }
        if (!g_animGraphUI.isCreatingLink && g_animGraphUI.resizingNodeId == node->id && ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
    ...
    for (const auto& link : graph->links) {
        ...
        // 3. Draw moving data flow particles
        // 3. Draw moving data flow particles (only if the link is active in the current animation frame)
        if (isRuntimeActive) {
            double animTime = ImGui::GetTime();
            float flowSpeed = 0.4f; // softer speed
            float progress = fmodf(static_cast<float>(animTime * flowSpeed), 1.0f);
            for (int k = 0; k < 2; k++) {
                float t_flow = progress + k * 0.5f;
                if (t_flow > 1.0f) t_flow -= 1.0f;
                float u_flow = 1.0f - t_flow;
                float wf1 = u_flow*u_flow*u_flow;
                float wf2 = 3.0f*u_flow*u_flow*t_flow;
                float wf3 = 3.0f*u_flow*t_flow*t_flow;
                float wf4 = t_flow*t_flow*t_flow;
                ImVec2 dotPos(
                    wf1*p1.x + wf2*cp1.x + wf3*cp2.x + wf4*p2.x,
                    wf1*p1.y + wf2*cp1.y + wf3*cp2.y + wf4*p2.y
                );
                ImU32 haloCol = (linkColor & 0x00FFFFFF) | 0x24000000; // soft halo
                drawList->AddCircleFilled(dotPos, 4.0f * zoom, haloCol);
                drawList->AddCircleFilled(dotPos, 1.5f * zoom, IM_COL32(255, 255, 255, 150)); // smaller and softer dot
            }
        }
    ...
    // ========== CANCEL / AUTO-CONNECT LINK CREATION ==========
    if (g_animGraphUI.isCreatingLink) {
        if (ImGui::IsMouseClicked(ImGuiMouseButton_Right) || ImGui::IsKeyPressed(ImGuiKey_Escape)) {
            g_animGraphUI.isCreatingLink = false;
            g_animGraphUI.linkStartPinId = 0;
            g_animGraphUI.pendingAutoConnectPinId = 0;
        }
        
        // Complete on release over empty space -> trigger Node Picker popup for auto connect
        if (ImGui::IsMouseReleased(ImGuiMouseButton_Left)) {
            if (g_animGraphUI.linkStartPinId != 0) {
                g_animGraphUI.pendingAutoConnectPinId = g_animGraphUI.linkStartPinId;
                g_animGraphUI.pendingAutoConnectIsOutput = g_animGraphUI.linkStartIsOutput;
    ...
    // ========== NODE & GROUP DRAGGING (only if not creating link) ==========
    if (!g_animGraphUI.isCreatingLink && ImGui::IsWindowHovered() && ImGui::IsMouseDragging(ImGuiMouseButton_Left)) {
        ...
        } else if (g_animGraphUI.resizingGroupId != 0) {
            auto* group = graph->getGroup(g_animGraphUI.resizingGroupId);
            if (group) {
        ...
        } else if (g_animGraphUI.draggingGroupId != 0) {
            auto* group = graph->getGroup(g_animGraphUI.draggingGroupId);
            if (group) {
                ...
                for (uint32_t nid : group->nodeIds) {
                    auto* node = graph->findNodeById(nid);
                    if (node) {
                        node->x += dx;
                        node->y += dy;
                    }
    ...
    if (!ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
        g_animGraphUI.resizingNodeId = 0;
        g_animGraphUI.resizingGroupId = 0;
        g_animGraphUI.draggingGroupId = 0;
        
        // Handle drag-and-drop grouping (release event)
        if (ImGui::IsMouseReleased(ImGuiMouseButton_Left) && !g_animGraphUI.selectedNodeIds.empty()) {
            for (uint32_t nid : g_animGraphUI.selectedNodeIds) {
                auto* node = graph->findNodeById(nid);
                if (node) {
    ...
    // ========== NODE & GROUP SELECTION & BOX SELECTION ==========
    if (!g_animGraphUI.isCreatingLink && g_animGraphUI.resizingNodeId == 0 &&
        g_animGraphUI.resizingGroupId == 0 && g_animGraphUI.draggingGroupId == 0 &&
        !interactingWithNodeChrome && ImGui::IsWindowHovered()) {
        
        if (ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
            bool clickedOnNode = false;
            bool clickedOnPin = false;
            bool clickedOnGroup = false;
            ...
            // Check if clicked on a group title
            for (auto& group : graph->groups) {
                ImVec2 pos = ImVec2(
                    canvasPos.x + g_animGraphUI.canvasOffset.x + group.position.x * zoom,
                    canvasPos.y + g_animGraphUI.canvasOffset.y + group.position.y * zoom);
                ...
                ImVec2 badgeMin = pos;
                ImVec2 badgeMax = ImVec2(pos.x + 120.0f * zoom, pos.y + 24.0f * zoom);
                if (mousePos.x >= badgeMin.x && mousePos.x <= badgeMax.x &&
            ...
            if (!clickedOnPin && !clickedOnGroup) {
                for (auto& node : graph->nodes) {
                    ImVec2 nodePos = ImVec2(
                        canvasPos.x + g_animGraphUI.canvasOffset.x + node->x * zoom,
                        canvasPos.y + g_animGraphUI.canvasOffset.y + node->y * zoom);
                ...
                if (!clickedOnNode) {
                    g_animGraphUI.selectedGroupId = 0;
                    if (!ImGui::GetIO().KeyCtrl) {
                        g_animGraphUI.selectedNodeIds.clear();
                    }
                    g_animGraphUI.isBoxSelecting = true;
                    g_animGraphUI.boxSelectStartPos = mousePos;
                }
            }
        }
    }

    // Box selection rendering and logic resolution
    if (g_animGraphUI.isBoxSelecting) {
        ImVec2 curMouse = ImGui::GetMousePos();
        drawList->AddRect(g_animGraphUI.boxSelectStartPos, curMouse, IM_COL32(255, 180, 50, 180), 0.0f, 0, 1.0f);
        drawList->AddRectFilled(g_animGraphUI.boxSelectStartPos, curMouse, IM_COL32(255, 180, 50, 20));
        
        if (ImGui::IsMouseReleased(ImGuiMouseButton_Left)) {
            g_animGraphUI.isBoxSelecting = false;
            
            float minX = std::min(g_animGraphUI.boxSelectStartPos.x, curMouse.x);
            float maxX = std::max(g_animGraphUI.boxSelectStartPos.x, curMouse.x);
            float minY = std::min(g_animGraphUI.boxSelectStartPos.y, curMouse.y);
            float maxY = std::max(g_animGraphUI.boxSelectStartPos.y, curMouse.y);
            ...
            if (std::abs(maxX - minX) > 4.0f || std::abs(maxY - minY) > 4.0f) {
                for (auto& node : graph->nodes) {
                    ImVec2 nodePos = ImVec2(
                        canvasPos.x + g_animGraphUI.canvasOffset.x + node->x * zoom,
                        canvasPos.y + g_animGraphUI.canvasOffset.y + node->y * zoom);
                    ImVec2 center(nodePos.x + 80.0f * zoom, nodePos.y + 40.0f * zoom);
                    
                    if (center.x >= minX && center.x <= maxX &&
                        center.y >= minY && center.y <= maxY) {
                        if (std::find(g_animGraphUI.selectedNodeIds.begin(), g_animGraphUI.selectedNodeIds.end(), node->id) == g_animGraphUI.selectedNodeIds.end()) {
                            g_animGraphUI.selectedNodeIds.push_back(node->id);
                        }
                    }
                }
    ...
    if (ImGui::IsWindowHovered() && ImGui::IsMouseClicked(ImGuiMouseButton_Right) && !g_animGraphUI.isCreatingLink) {
        ...
        // Check if right-clicked on a group title
        for (auto& group : graph->groups) {
            ImVec2 pos = ImVec2(
                canvasPos.x + g_animGraphUI.canvasOffset.x + group.position.x * zoom,
                canvasPos.y + g_animGraphUI.canvasOffset.y + group.position.y * zoom);
    ...
    if (ImGui::BeginPopup("AddNodePopup")) {
        // Group options if nodes are selected
        if (!g_animGraphUI.selectedNodeIds.empty()) {
            if (ImGui::MenuItem("Create Group Frame from Selection", "Ctrl+G")) {
                float minX = FLT_MAX, minY = FLT_MAX, maxX = -FLT_MAX, maxY = -FLT_MAX;
                for (uint32_t nid : g_animGraphUI.selectedNodeIds) {
                    auto* n = graph->findNodeById(nid);
                    if (n) {
        ...
        if (ImGui::MenuItem("Create Empty Group Frame")) {
            float graphX = (g_animGraphUI.addNodePopupPos.x - canvasPos.x - g_animGraphUI.canvasOffset.x) / zoom;
            float graphY = (g_animGraphUI.addNodePopupPos.y - canvasPos.y - g_animGraphUI.canvasOffset.y) / zoom;
        ...
        for (const auto& [typeId, displayName] : nodeTypes) {
            if (ImGui::MenuItem(displayName.c_str())) {
                ...
                if (newNode) {
                    ...
                    // Auto-connect if node was created from dragging a link cable into empty space
                    if (g_animGraphUI.pendingAutoConnectPinId != 0) {
                        if (g_animGraphUI.pendingAutoConnectIsOutput) {
                            if (!newNode->inputs.empty()) {
                                graph->connect(g_animGraphUI.pendingAutoConnectPinId, newNode->inputs[0].id);
                            }
                        } else {
                            if (!newNode->outputs.empty()) {
                                graph->connect(newNode->outputs[0].id, g_animGraphUI.pendingAutoConnectPinId);
                            }
                        }
                        g_animGraphUI.pendingAutoConnectPinId = 0;
                    }
    ...
    if (ImGui::BeginPopup("LocalNodeContextPopup")) {
        if (!g_animGraphUI.selectedNodeIds.empty()) {
            uint32_t activeNodeId = g_animGraphUI.selectedNodeIds.front();
            auto* activeNode = graph->findNodeById(activeNodeId);
            if (activeNode) {
    ...
    if (ImGui::BeginPopup("LocalGroupContextPopup")) {
        auto* group = graph->getGroup(g_animGraphUI.selectedGroupId);
        if (group) {
            ...
            if (ImGui::MenuItem("Select All Nodes in Group")) {
                g_animGraphUI.selectedNodeIds = group->nodeIds;
            }
    ...
    if (ImGui::IsWindowFocused(ImGuiFocusedFlags_ChildWindows) && ImGui::GetIO().KeyCtrl && ImGui::IsKeyPressed(ImGuiKey_G)) {
        if (!g_animGraphUI.selectedNodeIds.empty()) {
            float minX = FLT_MAX, minY = FLT_MAX, maxX = -FLT_MAX, maxY = -FLT_MAX;
            for (uint32_t nid : g_animGraphUI.selectedNodeIds) {
                auto* n = graph->findNodeById(nid);
            ...
            if (minX != FLT_MAX) {
                uint32_t gid = graph->createGroup("Group " + std::to_string(graph->groups.size() + 1), 
                    ImVec2(minX - 16.0f, minY - 32.0f), 
                    ImVec2(maxX - minX + 32.0f, maxY - minY + 48.0f));
                for (uint32_t nid : g_animGraphUI.selectedNodeIds) {
                    graph->addNodeToGroup(nid, gid);
...
inline void drawAnimationGraphPanel(UIContext& ctx) {
    ...
    // Toolbar (slim single row layout at top)
    {
        ...
        if (activeModelCtx) {
            useAnimGraph = activeModelCtx->useAnimGraph;
        ...
        if (UIWidgets::IconActionButton("AnimGraphSyncScene", UIWidgets::IconType::Duplicate, "Sync Scene", false,
            ...
            for (const auto& model : ctx.scene.importedModelContexts) {
                RigAuthoring::synchronizeClipRuntime(ctx.scene, model.importName);
            }
            if (ctx.scene.importedModelContexts.empty()) {
                AnimationController::getInstance().registerClips(
    ...
    // ========== MAIN CONTENT: Left Panel | Resize Handle | Node Canvas | Right Panel ==========
    ...
    // Dynamically clamp panel widths to ensure node canvas gets at least 180px and right panel never overflows parent window
    float maxLeftWidth = std::max(minPanelWidth, availWidth - rightPanelWidth - 180.0f - 12.0f);
    leftPanelWidth = std::clamp(leftPanelWidth, minPanelWidth, std::min(maxPanelWidth, maxLeftWidth));

    float maxRightWidth = std::max(minPanelWidth, availWidth - leftPanelWidth - 180.0f - 12.0f);
    rightPanelWidth = std::clamp(rightPanelWidth, minPanelWidth, std::min(maxPanelWidth, maxRightWidth));
    ...
    if (ImGui::IsItemActive()) {
        ...
        float maxL = std::max(minPanelWidth, availWidth - rightPanelWidth - 180.0f - 12.0f);
        leftPanelWidth = std::clamp(leftPanelWidth, minPanelWidth, std::min(maxPanelWidth, maxL));
    }
    ...
    // Node canvas
    float canvasWidth = std::max(180.0f, availWidth - leftPanelWidth - rightPanelWidth - 12.0f);
    ...
    if (ImGui::IsItemActive()) {
        ...
        float maxR = std::max(minPanelWidth, availWidth - leftPanelWidth - 180.0f - 12.0f);
        rightPanelWidth = std::clamp(rightPanelWidth, minPanelWidth, std::min(maxPanelWidth, maxR));
    }
    ...
    // Right panel (fill remaining width up to right edge of parent window)
    ImGui::PushStyleColor(ImGuiCol_ChildBg, ImVec4(0.08f, 0.10f, 0.14f, 0.82f));
    ImGui::BeginChild("AnimRightPanel", ImVec2(0, availHeight), true, ImGuiWindowFlags_NoMove);
    ...
    if (currentGraph) {
        ...
        // Draw Selected Node Info compactly
        ImGui::TextDisabled("Selected:");
        ImGui::SameLine();
        ImGui::Text("%s", selectedNode ? selectedNode->name.c_str() : "(None)");
        
        ImGui::TextDisabled("Type: %s |", selectedNode ? selectedNode->getTypeId().c_str() : "-");
        ImGui::SameLine();
        ImGui::TextColored(runtimeStale ? ImVec4(1.0f, 0.55f, 0.35f, 1.0f) : ImVec4(0.45f, 0.9f, 0.55f, 1.0f),
        ...
        // Tabs dynamically sizing to fit panel width
        float tabWidth = std::max(50.0f, (ImGui::GetContentRegionAvail().x - 12.0f) / 3.0f);
        if (UIWidgets::HorizontalTab("Properties", UIWidgets::IconType::System, g_animGraphUI.rightPanelTab == 0, tabWidth)) g_animGraphUI.rightPanelTab = 0;
        ImGui::SameLine();
        if (UIWidgets::HorizontalTab("State", UIWidgets::IconType::Graph, g_animGraphUI.rightPanelTab == 1, tabWidth)) g_animGraphUI.rightPanelTab = 1;
        ImGui::SameLine();
        if (UIWidgets::HorizontalTab("Runtime", UIWidgets::IconType::AnimGraph, g_animGraphUI.rightPanelTab == 2, tabWidth)) g_animGraphUI.rightPanelTab = 2;
