...
struct AnimationData {
    RigAuthoring::IKChannels
        ikChannels; // Persistent world-space authoring channels, time in seconds.
    bool rigAuthoring =
        false; // Canonical editable bone-channel clip; serialized in native projects.
    std::string name;
    ...
    Matrix4x4 calculateAnimationTransform(const AnimationData &animData, float time,
        ...
        const double requestedTime = time * ticksPerSecond;
        double animationTime = loop
                                   ? fmod(requestedTime, duration)
                                   : (std::clamp)(requestedTime, 0.0, duration);
        ...
        auto keySegment = [&](const auto &keys) {
            KeySegment result;
            if (keys.size() < 2) {
                return result;
            }
            if (!loop) {
                if (animationTime <= keys.front().time) {
                    return result;
                }
                if (animationTime >= keys.back().time) {
                    result.current = keys.size() - 1;
                    result.next = result.current;
                    return result;
                }
                const auto upper =
                    std::upper_bound(keys.begin(), keys.end(), animationTime,
                                     [](double value, const auto &key) {
                                         return value < key.time;
                                     });
                result.current = static_cast<size_t>(upper - keys.begin() - 1);
                result.next = result.current + 1;
                const double span = keys[result.next].time - keys[result.current].time;
                if (span > 0.0) {
                    result.factor = (std::clamp)(
                        (animationTime - keys[result.current].time) / span, 0.0, 1.0);
                }
                return result;
            }
            const auto upper =
            ...
            result.current = upper == keys.begin() ? keys.size() - 1
                                                   : static_cast<size_t>(upper - keys.begin() - 1);
            result.next = (result.current + 1) % keys.size();
            double span = keys[result.next].time - keys[result.current].time;
            if (span <= 0) {
                span += duration;
            }
            double elapsed = animationTime - keys[result.current].time;
            if (elapsed < 0) {
                elapsed += duration;
            }
            if (span > 0) {
                result.factor = (std::clamp)(elapsed / span, 0.0, 1.0);
            }
            return result;
        };
        ...
        if (hasPos) {
            ...
            const auto segment = keySegment(keys);

            const Vec3 &start = keys[segment.current].value;
            const Vec3 &end = keys[segment.next].value;
            const float factor = static_cast<float>(segment.factor);
            position =
                Vec3(start.x + (end.x - start.x) * factor, start.y + (end.y - start.y) * factor,
        ...
        if (hasRot) {
            ...
            const Quaternion &start = keys[segment.current].value;
            const Quaternion &end = keys[segment.next].value;
            rotation = Quaternion::slerp(start, end, static_cast<float>(segment.factor));
...
struct BoneData {
    ...
    std::unordered_map<std::string, Matrix4x4>
        boneDefaultTransforms; // NEW: Local bind pose (node->mTransformation)
    std::unordered_map<std::string, std::string>
        boneParents; // NEW: Child name -> Parent name for hierarchy reconstruction
    std::unordered_map<std::string, Matrix4x4>
        perModelInverses; // NEW: Model prefix -> globalInverseTransform
    std::unordered_set<std::string>
        weightedBoneNames; // Only bones coming from skinned mesh weights
    ...
    size_t getBoneIndexCapacity() const {
        size_t capacity = 0;
        for (const auto &entry : boneNameToIndex) {
            const size_t end = static_cast<size_t>(entry.second) + 1;
            if (end > capacity)
                capacity = end;
        }
        return capacity;
