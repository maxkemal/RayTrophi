...
// RayTrophi's own animation key types. AnimationData below stores THESE,
// not aiVectorKey/aiQuatKey - see Animation/AnimationKeys.h for why that
// had to change before any non-Assimp reader could exist.
#include "Animation/AnimationKeys.h"
// The node tree the animation runtime walks every frame. Built here at load
// time and COPIED OUT — the runtime must not need the aiScene (or this
// loader) to stay alive. See Animation/NodeHierarchy.h.
#include "Animation/NodeHierarchy.h"
// AnimationData + BoneData. Core types; they lived in THIS file until Faz 3.
...
// ---------------------------------------------------------------------------
// THE ONLY PLACE aiVectorKey / aiQuatKey ARE ALLOWED TO EXIST.
//
// Faz 0 of the Assimp import replacement moved AnimationData onto RayTrophi's
// own key types, so the conversion happens once, here, at the loader boundary.
// Everything downstream (AnimationController, AnimationNodes, OzzRuntime, the
// .rtp serializer, the timeline widget, the glTF writer) now speaks only
// RayTrophi::VectorKey / QuatKey.
//
// ★ Note the quaternion component order: aiQuaternion is (w, x, y, z) and so is
// this engine's Quaternion constructor. Getting this backwards produces a
// rotation that is wrong but still normalised - it animates, it just animates
// incorrectly, which is the kind of failure nobody files as a bug.
// ---------------------------------------------------------------------------
inline void appendAnimationKeys(RayTrophi::VectorKeys& dst, const aiVectorKey* src, unsigned int count) {
    dst.reserve(dst.size() + count);
    for (unsigned int i = 0; i < count; ++i) {
        dst.emplace_back(src[i].mTime, Vec3(src[i].mValue.x, src[i].mValue.y, src[i].mValue.z));
    }
}

inline void appendAnimationKeys(RayTrophi::QuatKeys& dst, const aiQuatKey* src, unsigned int count) {
    dst.reserve(dst.size() + count);
    for (unsigned int i = 0; i < count; ++i) {
        dst.emplace_back(src[i].mTime,
                         Quaternion(src[i].mValue.w, src[i].mValue.x, src[i].mValue.y, src[i].mValue.z));
    }
}

template <typename KeyVec, typename AiKey>
inline void assignAnimationKeys(KeyVec& dst, const AiKey* src, unsigned int count) {
    dst.clear();
    appendAnimationKeys(dst, src, count);
}
...
// ★ AnimationData and BoneData USED TO BE DEFINED HERE. They are core types and
// now live in Animation/AnimationData.h (included above), so that a reader which
// replaces Assimp does not have to include Assimp's loader to describe a
// skeleton. MeshInstance was also defined here and was deleted: no readers.
...
public:
    ...
    // Filled during load, then copied into ImportedModelContext. This is the
    // ONLY thing the per-frame animation walk needed from the aiScene, so once
    // it is copied out the loader no longer has to outlive the import.
    ...
    std::tuple<std::vector<std::shared_ptr<Triangle>>, std::vector<std::shared_ptr<AnimationData>>, BoneData>
        ...
        // ★ One walk builds BOTH: the aiNode lookup this loader still uses
        // internally, and the RayTrophi-owned tree the animation runtime walks
        // every frame. Note uniqueName is resolved HERE — deriving it during the
        // per-frame walk is what forced a loader pointer into that hot path.
        std::function<void(aiNode*, int)> recurse = [&](aiNode* node, int parentIndex) {
            const std::string originalName = node->mName.C_Str();
            nodeMap[originalName] = node;
            const int selfIndex = nodeHierarchy.addNode(
                originalName, getUniqueName(originalName), convert(node->mTransformation), parentIndex);
            for (unsigned int i = 0; i < node->mNumChildren; ++i)
                recurse(node->mChildren[i], selfIndex);
            };
        recurse(scene->mRootNode, -1);
        SCENE_LOG_INFO("Node map built with " + std::to_string(nodeMap.size()) + " nodes ("
                       + std::to_string(nodeHierarchy.size()) + " hierarchy nodes).");
            ...
            for (unsigned int i = 0; i < scene->mNumAnimations; ++i) {
                ...
                for (unsigned int j = 0; j < animation->mNumChannels; ++j) {
                    ...
                    if (channel->mNumPositionKeys > 0) {
                        appendAnimationKeys(animData->positionKeys[nodeName],
                    ...
                    if (channel->mNumRotationKeys > 0) {
                        appendAnimationKeys(animData->rotationKeys[nodeName],
                    ...
                    if (channel->mNumScalingKeys > 0) {
                        appendAnimationKeys(animData->scalingKeys[nodeName],
        ...
        if (scene->mNumAnimations > 0) {
            for (unsigned int i = 0; i < scene->mNumAnimations; ++i) {
                ...
                for (unsigned int j = 0; j < animation->mNumChannels; ++j) {
                    const aiNodeAnim* channel = animation->mChannels[j];
                    std::string nodeName = getUniqueName(channel->mNodeName.C_Str());
                    // One pre-sized bulk conversion per channel — no incremental reallocs.
                    assignAnimationKeys(animData->positionKeys[nodeName],
                                        channel->mPositionKeys, channel->mNumPositionKeys);
                    assignAnimationKeys(animData->rotationKeys[nodeName],
                                        channel->mRotationKeys, channel->mNumRotationKeys);
                    assignAnimationKeys(animData->scalingKeys[nodeName],
                                        channel->mScalingKeys, channel->mNumScalingKeys);
    ...
    void clearTextureCache() {
      //  SCENE_LOG_INFO("[TEXTURE CLEANUP] Starting texture cache cleanup...");
        ...
        // 2. Global singleton cache'i de temizle. (Embedded texture'lar icin ayri
        // bir global metadata cache'i vardi; piksel tutmadigi icin "isabet"i
        // kullanilamaz bir texture uretiyordu -- kaldirildi, bkz. Texture.h.)
    ...
    // =========================================================================
    // EMBEDDED TEXTURE CACHE KEY
    // =========================================================================
    // Identifies one embedded texture of THIS import. It is both the dedup key
    // for this loader's textureCache and the texture's lasting identity
    // (Texture::name, which the project file and the exporter read back), so it
    // has to name the texture rather than merely be distinct. Two rules:
    //
    //  * currentImportName must be unique per source file. Every caller that
    //    passes an import_prefix owns that guarantee (FoliageAssets derives one
    //    from the asset's relative path); the default is folder + file stem.
    //  * the texture is addressed by its INDEX in aiScene::mTextures, never by
    //    the aiTexture pointer. That pointer dies with the aiScene and the
    //    allocator hands the same address straight back to the next import, so
    //    a pointer key aliases two unrelated textures and is not stable across
    //    sessions either -- it cannot serve as an identity.
    //
    // Both key sites (prefetchTextures and processMaterial) must call this;
    // a mismatch degrades to a re-decode, not to a wrong texture.
    std::string makeEmbeddedTextureKey(const aiScene* scene, const aiTexture* emb, TextureType ttype) const {
        std::string slot;
        if (scene && emb) {
            for (unsigned int i = 0; i < scene->mNumTextures; ++i) {
                if (scene->mTextures[i] == emb) { slot = std::to_string(i); break; }
            }
        }
        // No index means the texture is not owned by this scene's table; fall
        // back to the address and accept the ABA risk rather than collapsing
        // every such texture onto one key.
        if (slot.empty()) slot = "p" + std::to_string(reinterpret_cast<uintptr_t>(emb));
        return "embedded_" + currentImportName + "_" + slot + "_" +
    ...
    void prefetchTextures(const aiScene* scene) {
        ...
        for (unsigned int m = 0; m < scene->mNumMaterials; ++m) {
            ...
            for (auto type : kScanTypes) {
                ...
                if (emb) {
                    cacheKey = makeEmbeddedTextureKey(scene, emb, ttype);
                } else {
    ...
    // ★ calculateAnimatedNodeTransformsRecursive(aiNode*, ...) was REMOVED in
    // Faz 0.5. It ran EVERY FRAME on the live aiScene, which is the reason
    // ImportedModelContext had to keep this loader (and the whole aiScene)
    // alive for the session — and the reason a model loaded by any non-Assimp
    // reader would silently not animate. Its replacement walks the
    // RayTrophi-owned tree instead: RayTrophi::computeAnimatedGlobalTransforms
    // in Animation/NodeHierarchy.h, fed by `nodeHierarchy` above.
    // Two code paths were NOT kept side by side (CLAUDE.md rule 5).
    ...
    void processBonesForMeshes(
        ...
        for (size_t meshIdx = 0; meshIdx < meshes.size(); ++meshIdx) {
            ...
            for (unsigned int b = 0; b < ai_mesh->mNumBones; ++b) {
                ...
                // The aiNode is a LOCAL lookup now: what BoneData keeps is the
                // values read off it, not the pointer (the aiScene dies first).
                auto nodeIt = nodeMap.find(boneName);
                if (nodeIt != nodeMap.end() && nodeIt->second) {
   ...
   // std::vector<std::shared_ptr<Camera>> cameras;
    ...
    void processNodeToTriangles(aiNode* node, const aiScene* scene, std::vector<std::shared_ptr<Triangle>>& triangles, const BoneData& boneData, OptixGeometryData* geometry_data = nullptr) {
        ...
        for (unsigned int i = 0; i < node->mNumMeshes; i++) {
            ...
            // actually has textures — otherwise leave the deep_ptr null = 8 bytes).
            // Yeni eklenen tüm üçgenlere texture paketini kopyala
            const bool anyImportedTex =
                hit_data.has_albedo_tex || hit_data.has_roughness_tex || hit_data.has_normal_tex ||
                hit_data.has_metallic_tex || hit_data.has_specular_tex || hit_data.has_transmission_tex ||
                hit_data.has_opacity_tex || hit_data.has_emission_tex;
            if (anyImportedTex) {
                OptixGeometryData::TextureBundle importedBundle;
                importedBundle.albedo_tex = hit_data.albedo_tex;
                importedBundle.has_albedo_tex = hit_data.has_albedo_tex;
                importedBundle.roughness_tex = hit_data.roughness_tex;
                importedBundle.has_roughness_tex = hit_data.has_roughness_tex;
                importedBundle.normal_tex = hit_data.normal_tex;
                importedBundle.has_normal_tex = hit_data.has_normal_tex;
                importedBundle.metallic_tex = hit_data.metallic_tex;
                importedBundle.has_metallic_tex = hit_data.has_metallic_tex;
                importedBundle.specular_tex = hit_data.specular_tex;
                importedBundle.has_specular_tex = hit_data.has_specular_tex;
                importedBundle.transmission_tex = hit_data.transmission_tex;
                importedBundle.has_transmission_tex = hit_data.has_transmission_tex;
                importedBundle.opacity_tex = hit_data.opacity_tex;
                importedBundle.has_opacity_tex = hit_data.has_opacity_tex;
        ...
        const BoneData& boneData)
    {
        ...
        if (hasActualWeights) {
            ...
            RigAuthoring::canonicalizeSkinWeights(meshVertexWeights);
       ...
       float opacity = 1.0f;
           ...
           if (AI_SUCCESS == aiGetMaterialFloat(aiMat, AI_MATKEY_OPACITY, &opacity)) {
            ...
            for (const auto& texInfo : textureInfos)
            {
                ...
                if (emb) {
                    // Format: "embedded_<ImportName>_<texture slot>_<type>".
                    // See makeEmbeddedTextureKey: the import name must be unique
                    // per source file and the slot is an index, not an address.
                    const std::string embeddedKey = makeEmbeddedTextureKey(scene, emb, ttype);
