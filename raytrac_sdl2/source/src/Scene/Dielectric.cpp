...
__m256 Dielectric::scatter_packet(const RayPacket& r_in, const HitRecordPacket& rec, Vec3Packet& attenuation, RayPacket& scattered) const {
    __m256 one = _mm256_set1_ps(1.0f);
    __m256 zero = _mm256_setzero_ps();
    
    // 1. Gather IOR and determine refraction ratio
    __m256 cos_theta_in = Vec3SIMD::dot_product_8x(-r_in.dir_x, -r_in.dir_y, -r_in.dir_z, rec.normal_x, rec.normal_y, rec.normal_z);
    __m256 is_front = _mm256_cmp_ps(cos_theta_in, zero, _CMP_GT_OQ);
    
    __m256 ior_val = _mm256_set1_ps(ir[1]); // Use green channel IOR
    __m256 ratio = _mm256_blendv_ps(ior_val, _mm256_div_ps(one, ior_val), is_front);
    
    __m256 abs_cos_theta = _mm256_and_ps(cos_theta_in, _mm256_castsi256_ps(_mm256_set1_epi32(~0x80000000)));
    __m256 cos_theta = _mm256_min_ps(abs_cos_theta, one);
    __m256 sin_theta = _mm256_sqrt_ps(_mm256_max_ps(_mm256_sub_ps(one, _mm256_mul_ps(cos_theta, cos_theta)), zero));
    
    // 2. Schlick Reflectance
    auto reflectance_simd = [&](__m256 cos, __m256 ref_idx) {
        __m256 r0 = _mm256_div_ps(_mm256_sub_ps(one, ref_idx), _mm256_add_ps(one, ref_idx));
        r0 = _mm256_mul_ps(r0, r0);
        __m256 omc = _mm256_sub_ps(one, cos);
        __m256 omc2 = _mm256_mul_ps(omc, omc);
        __m256 omc5 = _mm256_mul_ps(_mm256_mul_ps(omc2, omc2), omc);
        return _mm256_add_ps(r0, _mm256_mul_ps(_mm256_sub_ps(one, r0), omc5));
    };
    
    __m256 reflect_prob = reflectance_simd(cos_theta, ratio);
    __m256 cannot_refract = _mm256_cmp_ps(_mm256_mul_ps(sin_theta, ratio), one, _CMP_GT_OQ);
    
    // 3. Random Lobe Selection
    alignas(32) float r_vals[8];
    for (int i = 0; i < 8; ++i) r_vals[i] = Vec3SIMD::random_float();
    __m256 rnd = _mm256_load_ps(r_vals);
    
    __m256 do_reflect = _mm256_or_ps(cannot_refract, _mm256_cmp_ps(rnd, reflect_prob, _CMP_LT_OQ));
    
    // 4. Trace rays
    Vec3SIMD refl_x, refl_y, refl_z;
    Vec3SIMD::reflect_8x(r_in.dir_x, r_in.dir_y, r_in.dir_z, rec.normal_x, rec.normal_y, rec.normal_z, refl_x, refl_y, refl_z);
    
    Vec3SIMD n_oriented_x = rec.normal_x;
    Vec3SIMD n_oriented_y = rec.normal_y;
    Vec3SIMD n_oriented_z = rec.normal_z;
    // Flip normal if we are exiting (not front face)
    __m256 sign_mask = _mm256_set1_ps(-0.0f);
    __m256 n_neg_x = _mm256_xor_ps(n_oriented_x.data, sign_mask);
    __m256 n_neg_y = _mm256_xor_ps(n_oriented_y.data, sign_mask);
    __m256 n_neg_z = _mm256_xor_ps(n_oriented_z.data, sign_mask);
    
    n_oriented_x.data = _mm256_blendv_ps(n_neg_x, n_oriented_x.data, is_front);
    n_oriented_y.data = _mm256_blendv_ps(n_neg_y, n_oriented_y.data, is_front);
    n_oriented_z.data = _mm256_blendv_ps(n_neg_z, n_oriented_z.data, is_front);
    
    Vec3SIMD refr_x, refr_y, refr_z;
    Vec3SIMD::refract_8x(r_in.dir_x, r_in.dir_y, r_in.dir_z, n_oriented_x, n_oriented_y, n_oriented_z, Vec3SIMD(ratio), refr_x, refr_y, refr_z);
    
    scattered.dir_x.data = _mm256_blendv_ps(refr_x.data, refl_x.data, do_reflect);
    scattered.dir_y.data = _mm256_blendv_ps(refr_y.data, refl_y.data, do_reflect);
    scattered.dir_z.data = _mm256_blendv_ps(refr_z.data, refl_z.data, do_reflect);
    
    float offset = 0.001f;
    scattered.orig_x.data = _mm256_add_ps(rec.p_x.data, _mm256_mul_ps(scattered.dir_x.data, _mm256_set1_ps(offset)));
    scattered.orig_y.data = _mm256_add_ps(rec.p_y.data, _mm256_mul_ps(scattered.dir_y.data, _mm256_set1_ps(offset)));
    scattered.orig_z.data = _mm256_add_ps(rec.p_z.data, _mm256_mul_ps(scattered.dir_z.data, _mm256_set1_ps(offset)));
    
    attenuation = Vec3Packet(color.x, color.y, color.z);
