...
void VDBVolume::hit_packet(const RayPacket& r, float t_min, float t_max, HitRecordPacket& rec, bool ignore_volumes) const {
    if (!visible || ignore_volumes || !isLoaded()) return;
    
    AABB world_box = getWorldBounds();
    
    // Vectorized AABB intersection
    __m256 t0_v = _mm256_set1_ps(t_min);
    __m256 t1_v = _mm256_set1_ps(t_max);
    
    // X axis
    __m256 t_near_x = _mm256_mul_ps(_mm256_sub_ps(_mm256_set1_ps(world_box.min.x), r.orig_x.data), r.inv_dir_x.data);
    __m256 t_far_x  = _mm256_mul_ps(_mm256_sub_ps(_mm256_set1_ps(world_box.max.x), r.orig_x.data), r.inv_dir_x.data);
    __m256 t_min_x = _mm256_min_ps(t_near_x, t_far_x);
    __m256 t_max_x = _mm256_max_ps(t_near_x, t_far_x);
    t0_v = _mm256_max_ps(t0_v, t_min_x);
    t1_v = _mm256_min_ps(t1_v, t_max_x);

    // Y axis
    __m256 t_near_y = _mm256_mul_ps(_mm256_sub_ps(_mm256_set1_ps(world_box.min.y), r.orig_y.data), r.inv_dir_y.data);
    __m256 t_far_y  = _mm256_mul_ps(_mm256_sub_ps(_mm256_set1_ps(world_box.max.y), r.orig_y.data), r.inv_dir_y.data);
    __m256 t_min_y = _mm256_min_ps(t_near_y, t_far_y);
    __m256 t_max_y = _mm256_max_ps(t_near_y, t_far_y);
    t0_v = _mm256_max_ps(t0_v, t_min_y);
    t1_v = _mm256_min_ps(t1_v, t_max_y);

    // Z axis
    __m256 t_near_z = _mm256_mul_ps(_mm256_sub_ps(_mm256_set1_ps(world_box.min.z), r.orig_z.data), r.inv_dir_z.data);
    __m256 t_far_z  = _mm256_mul_ps(_mm256_sub_ps(_mm256_set1_ps(world_box.max.z), r.orig_z.data), r.inv_dir_z.data);
    __m256 t_min_z = _mm256_min_ps(t_near_z, t_far_z);
    __m256 t_max_z = _mm256_max_ps(t_near_z, t_far_z);
    t0_v = _mm256_max_ps(t0_v, t_min_z);
    t1_v = _mm256_min_ps(t1_v, t_max_z);

    __m256 hit_mask = _mm256_cmp_ps(t0_v, t1_v, _CMP_LE_OQ);
    if (_mm256_movemask_ps(hit_mask) == 0) return;

    // We have some hits. Populate temp record.
    HitRecordPacket temp;
    temp.t.data = t0_v;
    r.point_at(temp.t, temp.p_x, temp.p_y, temp.p_z);
    
    // Normal calculation: point toward ray origin (negated direction)
    temp.normal_x.data = _mm256_sub_ps(_mm256_setzero_ps(), r.dir_x.data);
    temp.normal_y.data = _mm256_sub_ps(_mm256_setzero_ps(), r.dir_y.data);
    temp.normal_z.data = _mm256_sub_ps(_mm256_setzero_ps(), r.dir_z.data);
    Vec3SIMD::normalize_8x(temp.normal_x, temp.normal_y, temp.normal_z, temp.normal_x, temp.normal_y, temp.normal_z);

    for(int i=0; i<8; i++) temp.vdb_volume[i] = this;
    temp.mat_id = _mm256_set1_epi32(0xFFFF); // Special ID for VDB
    temp.mask = hit_mask;

    rec.merge_if_closer(temp, hit_mask);
}

__m256 VDBVolume::occluded_packet(const RayPacket& packet, float t_min, __m256 t_max) const {
    const RayPacket& r = packet; // Keep internal 'r' alias for compatibility
    if (!visible || !isLoaded()) return _mm256_setzero_ps();

    AABB world_box = getWorldBounds();
    
    // 1. Vectorized AABB intersection
    __m256 t0_v = _mm256_set1_ps(t_min);
    __m256 t1_v = t_max;
    
    // X
    __m256 tn_x = _mm256_mul_ps(_mm256_sub_ps(_mm256_set1_ps(world_box.min.x), r.orig_x.data), r.inv_dir_x.data);
    __m256 tf_x = _mm256_mul_ps(_mm256_sub_ps(_mm256_set1_ps(world_box.max.x), r.orig_x.data), r.inv_dir_x.data);
    t0_v = _mm256_max_ps(t0_v, _mm256_min_ps(tn_x, tf_x));
    t1_v = _mm256_min_ps(t1_v, _mm256_max_ps(tn_x, tf_x));

    // Y
    __m256 tn_y = _mm256_mul_ps(_mm256_sub_ps(_mm256_set1_ps(world_box.min.y), r.orig_y.data), r.inv_dir_y.data);
    __m256 tf_y = _mm256_mul_ps(_mm256_sub_ps(_mm256_set1_ps(world_box.max.y), r.orig_y.data), r.inv_dir_y.data);
    t0_v = _mm256_max_ps(t0_v, _mm256_min_ps(tn_y, tf_y));
    t1_v = _mm256_min_ps(t1_v, _mm256_max_ps(tn_y, tf_y));

    // Z
    __m256 tn_z = _mm256_mul_ps(_mm256_sub_ps(_mm256_set1_ps(world_box.min.z), r.orig_z.data), r.inv_dir_z.data);
    __m256 tf_z = _mm256_mul_ps(_mm256_sub_ps(_mm256_set1_ps(world_box.max.z), r.orig_z.data), r.inv_dir_z.data);
    t0_v = _mm256_max_ps(t0_v, _mm256_min_ps(tn_z, tf_z));
    t1_v = _mm256_min_ps(t1_v, _mm256_max_ps(tn_z, tf_z));

    __m256 hit_mask = _mm256_cmp_ps(t0_v, t1_v, _CMP_LE_OQ);
    if (_mm256_movemask_ps(hit_mask) == 0) return _mm256_setzero_ps();

    // 2. Stochastic Ray Marching (Scalar loop for robust sampling, matching occluded())
    alignas(32) int result[8] = {0};
    alignas(32) float t_enter[8], t_exit[8];
    _mm256_store_ps(t_enter, t0_v);
    _mm256_store_ps(t_exit, t1_v);

    float base_step = 0.5f;
    if (this->volume_shader) base_step = this->volume_shader->quality.step_size * 2.0f;
    float density_mult = (this->volume_shader ? this->volume_shader->density.multiplier : 1.0f) * this->density_scale;
    const auto& mgr = VDBVolumeManager::getInstance();
    int vid = this->getVDBVolumeID();
    Matrix4x4 inv = world_transform_inv;

    for (int i = 0; i < 8; i++) {
        if (!((1 << i) & _mm256_movemask_ps(hit_mask))) continue;

        Ray rs(Vec3(r.orig_x.get(i), r.orig_y.get(i), r.orig_z.get(i)),
               Vec3(r.dir_x.get(i), r.dir_y.get(i), r.dir_z.get(i)));
        
        float t = t_enter[i] + Vec3::random_float() * base_step;
        float tr = 1.0f;

        while (t < t_exit[i]) {
            Vec3 p = rs.at(t);
            Vec3 lp = inv.transform_point(p);
            float d = mgr.sampleDensityCPU(vid, lp.x, lp.y, lp.z);
            if (d > 0.001f) {
                tr *= exp(-d * density_mult * base_step);
            }
            if (tr < 0.01f) break;
            t += base_step;
        }

        if (Vec3::random_float() > tr) {
            result[i] = 0xFFFFFFFF;
        }
    }

    return _mm256_load_ps((float*)result);
}
