/**
 * @file VDBVolume.cpp
 * @brief Implementation of VDB Volume scene object
 */
...
bool VDBVolume::loadVDB(const std::string& path) {
    ...
    // Auto-Rotate for Blender coordinate system (Z-up to Y-up)
...
void VDBVolume::updateFromTimeline(int timeline_frame) {
    ...
    bool loaded = VDBVolumeManager::getInstance().updateVolume(vdb_volume_id, new_path);
    if (loaded) {
...
bool VDBVolume::loadVDBSequence(const std::string& pattern_or_file) {
    ...
    if (pattern_or_file.find("#") != std::string::npos) {
         ...
         char buf[16];
         snprintf(buf, 16, "%04d", min_frame);
         std::string path = sequence_pattern;
         path.replace(path.find("####"), 4, buf);
...
bool VDBVolume::hit(const Ray& r, float t_min, float t_max, HitRecord& rec) const {
...
void VDBVolume::centerPivotToBottomCenter() {
    ...
    // Use current local_bbox which (if this is first call) is raw bounds
    Vec3 center = (local_bbox_min + local_bbox_max) * 0.5f;
    
    // Calculate offset needed to move center to (0,0,0) with Y at bottom
    Vec3 offset_needed;
    offset_needed.x = -center.x;
    offset_needed.y = -local_bbox_min.y;
    offset_needed.z = -center.z;
    
    // Store in persistent member so we can apply to future frames
    // Accumulate in case called multiple times (though usually once)
    pivot_offset += offset_needed;
    
    // Apply immediate offset to current bounds
    local_bbox_min += offset_needed;
    local_bbox_max += offset_needed;
    
    // Also update first frame bounds for sequences
...
void VDBVolume::updateTransformMatrix() {
    ...
    Matrix4x4 Rx = Matrix4x4::rotationX(rotation_euler.x * 3.14159265358979 / 180.0);
    Matrix4x4 Ry = Matrix4x4::rotationY(rotation_euler.y * 3.14159265358979 / 180.0);
    Matrix4x4 Rz = Matrix4x4::rotationZ(rotation_euler.z * 3.14159265358979 / 180.0);
...
void VDBVolume::updateBoundsFromVDB() {
    ...
    if (vdb_data) {
        ...
        // Validating bounding box values to prevent overflow corruption (e.g. 2^32)
        const float MAX_VALID_BBOX = 1e9f;
        bool bbox_valid = true;
        
        if (std::abs(raw_bbox_min.x) > MAX_VALID_BBOX || std::abs(raw_bbox_min.y) > MAX_VALID_BBOX || std::abs(raw_bbox_min.z) > MAX_VALID_BBOX ||
            std::abs(raw_bbox_max.x) > MAX_VALID_BBOX || std::abs(raw_bbox_max.y) > MAX_VALID_BBOX || std::abs(raw_bbox_max.z) > MAX_VALID_BBOX ||
            std::isnan(raw_bbox_min.x) || std::isnan(raw_bbox_max.x) || std::isinf(raw_bbox_min.x) || std::isinf(raw_bbox_max.x)) {
            // Invalid bbox detected in sequence update - use default bounds
            raw_bbox_min = Vec3(-0.5);
            raw_bbox_max = Vec3(0.5);
            bbox_valid = false;
        }

        // Apply Persistent Pivot Offset (calculated in centerPivotToBottomCenter)
        // This ensures the pivot stays at bottom-center even when frame changes
        Vec3 new_bbox_min = raw_bbox_min + pivot_offset;
        Vec3 new_bbox_max = raw_bbox_max + pivot_offset;

        // For sequences: store first frame bounds and use for all frames
        // This ensures auto-scale (cm→m) stays consistent
        if (is_sequence) {
            if (!has_first_frame_bounds) {
                // First frame - store these bounds as reference
                ...
                // Subsequent frames - merge with first frame bounds
                // But only if current frame has valid bounds or use first frame bounds as fallback
                if (bbox_valid) {
                    
                    // LATE AUTO-SCALE DETECTION
                    // If we encounter a massive frame (e.g. > 50.0 units) but our scale is still ~1.0,
                    // It means the first frame was small/unit sized and we missed the auto-scale.
                    // We must correct it now to prevent giant explosion.
                    ...
                    if (max_d > 50.0f && scale_vec.x > 0.5f) {
                        float corrected_scale = 0.01f; // Force cm->m conversion
                        setScale(Vec3(corrected_scale));
                        SCENE_LOG_WARN("Late Auto-Scale triggered: Detected massive frame (" + std::to_string(max_d) + 
                                       "), forcing scale to 0.01");
                    }

                    local_bbox_min.x = std::min(first_frame_bbox_min.x, new_bbox_min.x);
                    local_bbox_min.y = std::min(first_frame_bbox_min.y, new_bbox_min.y);
                    local_bbox_min.z = std::min(first_frame_bbox_min.z, new_bbox_min.z);
                    local_bbox_max.x = std::max(first_frame_bbox_max.x, new_bbox_max.x);
                    local_bbox_max.y = std::max(first_frame_bbox_max.y, new_bbox_max.y);
                    local_bbox_max.z = std::max(first_frame_bbox_max.z, new_bbox_max.z);
                } else {
                    // If current frame is invalid, stick to first frame bounds
                    local_bbox_min = first_frame_bbox_min;
                    local_bbox_max = first_frame_bbox_max;
                }
            }
        } else {
            // Single VDB - just use current bounds
            local_bbox_min = new_bbox_min;
            local_bbox_max = new_bbox_max;
        }
    } else {
